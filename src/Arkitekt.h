...
|.arch x86 |.section code |.globals lbl_ |.actionlist compiler_actions struct Action;
namespace Arkitekt
{
    struct FnBlock;
    struct Assembler;
...
struct push : public _Ins
{
    void operator()(Arkitekt::Assembler *ptr, Operand auto toPush)
    {
        ...
        {
            // immediate pop
            | push aword[toPush.value] ptr->stackOffset += sizeof(uintptr_t);
        }
...
struct add : public _Ins
{
    void operator()(Arkitekt::Assembler *ptr, is_register auto target, imm32 auto value)
    {
        ...
        {
        case BITMASK_R16:
            | add Rw(target.ToRegID()), (ushort)uint32_t(value) break;
        case BITMASK_R32:
            | add Rd(target.ToRegID()), uint32_t(value) break;
        case BITMASK_R8:
            | add Rb(target.ToRegID()), (unsigned char)uint32_t(value) break;
        case BITMASK_XMM: // invalid in 32bit mode(?)
            throw new std::bad_function_call;
            break;
        }
...
struct call : public _Ins
{
    void operator()(Arkitekt::Assembler *ptr, Operand auto address)
    {
        ...
        else if constexpr (std::same_as<std::remove_all_extents_t<decltype(address)>, RegisterIndex>)
        {
            ...
            | push = > counter | push dword[Rd(address.ToRegID()) + address.GetValue()] | ret |= > counter:
        }
...
struct mov : public _Ins
{
    void operator()(Arkitekt::Assembler *ptr, Operand auto lhs, Operand auto rhs)
    {
        ...
        if constexpr (std::same_as<decltype(lhs), RegisterIndex>)
        {
            ...
            {
                | push aword[Rd(rhs.ToRegID()) + rhs.GetValue()] | pop aword[Rd(lhs.ToRegID()) + lhs.GetValue()]
            }
            ...
            {
                | push aword[Rd(lhs.ToRegID()) + lhs.GetValue()] | pop aword[uint32_t(rhs)]
            }
        }
        ...
        else if constexpr (imm32<decltype(lhs)> || is_address<decltype(lhs)>)
        {
            if constexpr (std::same_as<std::remove_all_extents_t<decltype(rhs)>, RegisterIndex>)
            {
                | push aword[uint32_t(lhs)] | pop aword[Rd(rhs.ToRegID()) + rhs.GetValue()]
            }
            else if constexpr (is_register<decltype(rhs)>)
            {
...
struct pop : public _Ins
{
    void operator()(Arkitekt::Assembler *ptr, Operand auto toPop)
    {
        ...
        {
            // immediate pop
            | pop aword[toPop.value] ptr->stackOffset -= sizeof(uintptr_t);
        }
        else if constexpr (std::same_as<std::remove_all_extents_t<decltype(toPop)>, RegisterIndex>)
        {
            if (0 < toPop.GetValue())
                | pop aword[Rd(toPop.ToRegID()) + toPop.GetValue()] else | pop aword[Rd(toPop.ToRegID()) - toPop.GetValue()]
        ...
        {
            | pop dword[uint32_t(toPop)] ptr->stackOffset -= sizeof(uint32_t);
        }
    }
    constexpr pop() {}
};
